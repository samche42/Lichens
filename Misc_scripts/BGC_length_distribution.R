...
data = read.csv("Fungal_BGC_length_distribution.txt", header=TRUE, sep ="\t")

order_levels <- c("T1PKS","NRPS-like","terpene","NRPS","NRPS_T1PKS hybrid","T3PKS","indole","NRPS-like_T1PKS hybrid","other","T1PKS_T3PKS hybrid","T1PKS_indole hybrid","NRPS_NRPS-like hybrid","T1PKS_terpene hybrid","betalactone","siderophore","NRPS_indole hybrid","fungal-RiPP","NRPS_T1PKS_terpene hybrid","NRPS-like_betalactone hybrid","NRPS_NRPS-like_T1PKS hybrid","Other")
...
ggplot(subset(data, Contig_edge == "YES"), aes(x = BGC_type, y = BGC_length, fill = BGC_type)) +
  ...
  scale_y_continuous(breaks = seq(0, 125000, by = 25000))
