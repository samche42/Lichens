...
accessions = [(((os.path.splitext(file))[0]).split("_"))[0] for file in os.listdir(input_directory) if file.endswith("Bacteria_3000bp_removed.fasta")]
...
for acc in accessions:
    ...
    fasta_file = acc+"_Bacteria_3000bp_removed.fasta"
...
final_count_df.to_csv("Bacteria_phyla_counts.txt", sep='\t', index=False)
final_abund_df.to_csv("Bacteria_phyla_abundances.txt", sep='\t', index=False)
final_length_df.to_csv("Bacteria_phyla_lengths.txt", sep='\t', index=False)
final_abund2_df.to_csv("Bacteria_phyla_abundances_over_3000bp.txt", sep='\t', index=False)
